import getopt, argparse, subprocess, os

"""
This function reads in the full unannotated VCF and chunks this in chunks of 25.000 variants.
For every chunk the original header is added to the variants.
The old chunks are then removed.
"""
def chunkVCF(inputdb, inputvcf, outputfile, queue, time, email, temp):
    ...
    createShells(inputdb, chunk_path, outputfile, queue, email, time, temp)

"""
This function creates shell scripts that will run the processing part of CADDtoAnno for every chunk.
Also creates a concat shell script that will merge the chunks after finishing annotating.
"""
def createShells(inputdb, chunk_path, outputfile, queue, email, time, temp):
    job_names = []
    concat_names = []
	...
	if files.startswith("headerchunk") and files.endswith(".vcf"):
	    ...
	    sh_script.write("time python {path}/CADDtoAnno_processingscript.py -db {db} -v {chunkpath}/{chunkfile} -o {chunkpath}/{vcfname}".format(
			    path = script_path,
			    db = inputdb,
...
if __name__ == "__main__":
    ...
    chunkVCF(args.database_file, args.vcf_file, args.output_file, args.queue, args.time, args.email, args.temp_directory)